...
async def test():
    async with UnitOfWork() as uow:
        ...
        membership = (await uow.session.execute(select(Membership).where(Membership.org_id == org.id).limit(1))).scalar_one_or_none()
        ...
        try:
            ...
            async with UnitOfWork() as uow2:
                job = (await uow2.session.execute(
                    select(DocsAIGenerationJob).where(DocsAIGenerationJob.id == job_id)
                )).scalar_one_or_none()
