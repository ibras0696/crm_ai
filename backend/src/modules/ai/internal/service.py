...
async def _build_org_context_internal(
    ...
    try:
        ...
        # Контекст — вспомогательная часть. Ошибка не должна ломать чат целиком,
        # но в проде важно видеть причину деградации.
...
async def _create_records_for_table(
    ...
    # Защита от гонок по position: сериализуем добавление записей по таблице.
    # Лочим родительскую строку таблицы, чтобы параллельные транзакции не выбрали
    # одинаковый max(position).
...
async def get_or_create_session(
    ...
    if chat_id:
        try:
            ...
            # Невалидный UUID или проблемы чтения из БД. Не ломаем запрос, а создаём новый чат.
...
async def handle_create_columns_action(
    ...
    # Защита от гонок по position: лочим таблицу на время добавления колонок.
    await uow.session.execute(select(Table.id).where(Table.id == table_obj.id).with_for_update())
