...
export default function PublicContentPage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background text-foreground">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(37,99,235,0.14),transparent_36%),radial-gradient(circle_at_bottom_right,rgba(34,211,238,0.12),transparent_34%)]" />
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col gap-12 px-4 py-10 md:px-8 md:py-16">
        <div className="flex items-center justify-between gap-4">
          <Link
            to="/landing"
            className="inline-flex items-center gap-2 rounded-full border border-border bg-card/80 px-4 py-2 text-sm font-semibold text-foreground/80 transition hover:border-primary/30 hover:bg-card hover:text-foreground"
          >
        ...
        <section className="grid gap-8 md:grid-cols-[1.3fr_0.7fr]">
          <div className="space-y-6">
            ...
            <p className="max-w-3xl text-lg leading-8 text-muted-foreground md:text-xl">{page.description}</p>
          </div>
          <div className="rounded-[2rem] border border-border bg-card/80 p-6 backdrop-blur">
        ...
        <section className="grid gap-6 md:grid-cols-3">
          {page.sections.map((section) => (
            <article
              key={section.heading}
              className="rounded-[2rem] border border-border bg-card/80 p-6 shadow-[0_18px_60px_-32px_rgba(56,189,248,0.28)] backdrop-blur"
            >
        ...
        <section className="rounded-[2rem] border border-border bg-gradient-to-r from-primary/10 via-card/90 to-cyan-400/15 p-8">
